// Copyright (c) 2014 The vTorrent developers
...
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/vtorrent"));
    setWindowIcon(QIcon(":icons/vtorrent"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/vtorrent/index.html");
#else
    QFile html("/opt/vtorrent/index.html");
#endif
...
void GUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("vTorrent") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/vtorrent_testnet"));
            setWindowIcon(QIcon(":icons/vtorrent_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/vtorrent_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("vTorrent client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/vtorrent_testnet"));
...
void GUI::createTrayIcon()
{
    ...
    trayIcon->setIcon(QIcon(":/icons/vtorrent"));
...
void GUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("data-title", tr("%n active connection(s) to vTorrent network", "", count));
}
    ...
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          ...
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::VTR, nFeeRequired));
