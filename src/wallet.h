...
public:
    ...
    bool SendVTRToToken(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendTokenToVTR(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nVTR, int64_t& nToken) const
    {
        nVTR = 0;
        nToken = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    bool GetCredit(int64_t& nCredVTR, int64_t& nCredToken, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredVTR = nCredToken = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredVTRCached = 0;
            nCredTokenCached = 0;
            pwallet->GetCredit(*this, nCredVTRCached, nCredTokenCached);
